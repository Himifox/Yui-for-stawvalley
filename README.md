...
<p>
  <img alt="Version 0.29.0" src="https://img.shields.io/badge/version-0.29.0-ec8fb4?style=flat-square">
...
> [!NOTE]
> 当前版本为 `0.29.0` 开发版本。Release 构建通过只代表代码可以编译，不代表全部实验能力已经完成游戏内验收。
...
工作执行器由注册表路由，更新和取消通过统一运行时模块广播。持久化模型按业务域拆分，同时保留现有 `schema-v9` 字段以兼容已有存档。
