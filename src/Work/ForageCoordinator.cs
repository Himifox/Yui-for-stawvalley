...
internal sealed class ForageCoordinator
{
    ...
    private static Vector2? FindApproachTile(GameLocation location, Vector2 target, NPC body)
    {
        ...
        return candidates.Where(candidate => location.isTileLocationOpen(candidate)
                && location.characters.All(character => ReferenceEquals(character, body) || character.Tile != candidate))
            .OrderBy(candidate => ManhattanDistance(candidate.ToPoint(), body.TilePoint)).Cast<Vector2?>().FirstOrDefault();
    }
